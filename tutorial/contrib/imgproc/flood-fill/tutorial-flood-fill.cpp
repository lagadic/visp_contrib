...
int main() {
  ...
  for (int i = 0; i < 3; i++) {
    vpDisplay::display(I);
    std::stringstream ss;
    ss << "Left click to draw polygon " << i+1 << "/3" << ", right click to close the shape.";
  ...
  while (!quit) {
    vpDisplay::display(I);
    vpDisplay::displayText(I, 20, 20, "Left click on a pixel location to fill the shape, right click to quit.", vpColor::red);
    vpDisplay::flush(I);
